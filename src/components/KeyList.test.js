...
describe("KeysList component", () => {
    ...
    //   test("navigates to add-key page on clicking Add button", async () => {
    //     renderKeysListWithUserContext({ setUser }, { setEditItem: jest.fn() });

    //     await waitFor(() => expect(getPassKeys).toHaveBeenCalledTimes(1));

    //     fireEvent.click(screen.getByText(/Add/i));
    //     await waitFor(() => {
    //       expect(screen.getByText(/SecureKey/)).toBeInTheDocument(); // Verifies the Navbar is still rendered
    //       expect(screen.queryByText(/Accounts/i)).not.toBeInTheDocument(); // Ensures we navigated away
    //     });
    //   });

    //   test("calls deletePassKey and updates keys list on delete", async () => {
    //     renderKeysListWithUserContext({ setUser }, { setEditItem: jest.fn() });
    ...
    //     expect(deletePassKey).toHaveBeenCalledWith({ userContext: { setUser }, account: "test1", username: "user1" });

    //     await waitFor(() => expect(getPassKeys).toHaveBeenCalledTimes(2)); // fetchPassKeys should be called again after deletion
    //   });
});
